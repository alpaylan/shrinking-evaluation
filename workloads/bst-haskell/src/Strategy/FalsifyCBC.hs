{-# LANGUAGE FlexibleInstances #-}
{-# LANGUAGE TemplateHaskell #-}

module Strategy.FalsifyCBC where

import Etna.Lib
import Impl
import Spec
import qualified Test.Falsify.Generator as Gen
import Test.Falsify.Generator (Gen)
import qualified Test.Falsify.Range as Range

-- Correct-by-construction BST generator (Falsify flavour). Mirrors
-- Strategy.QuickCBC / Strategy.HedgehogCBC.
genBSTCBCF :: Int -> Int -> Int -> Gen BST
genBSTCBCF depth lo hi
  | depth <= 0 || lo + 1 >= hi = pure E
  | otherwise =
      Gen.frequency
        [ (1, pure E)
        , ( 3
          , do
              k <- Gen.int (Range.between (lo + 1, hi - 1))
              v <- Gen.int (Range.withOrigin (-1000, 1000) 0)
              left <- genBSTCBCF (depth - 1) lo k
              right <- genBSTCBCF (depth - 1) k hi
              pure (T left (Key k) (Val v) right)
          )
        ]

class FGen a where
  fgen :: Gen a

instance FGen BST where
  fgen = genBSTCBCF 5 (-1000) 1000

instance FGen Key where
  fgen = Key <$> Gen.int (Range.withOrigin (-1000, 1000) 0)

instance FGen Val where
  fgen = Val <$> Gen.int (Range.withOrigin (-1000, 1000) 0)

instance (FGen a, FGen b) => FGen (a, b) where
  fgen = (,) <$> fgen <*> fgen

instance (FGen a, FGen b, FGen c) => FGen (a, b, c) where
  fgen = (,,) <$> fgen <*> fgen <*> fgen

instance (FGen a, FGen b, FGen c, FGen d) => FGen (a, b, c, d) where
  fgen = (,,,) <$> fgen <*> fgen <*> fgen <*> fgen

instance (FGen a, FGen b, FGen c, FGen d, FGen e) => FGen (a, b, c, d, e) where
  fgen = (,,,,) <$> fgen <*> fgen <*> fgen <*> fgen <*> fgen

$( mkStrategies
     [|fsRunGen fsDefaults Naive fgen|]
     [ 'prop_InsertValid,
       'prop_DeleteValid,
       'prop_UnionValid,
       'prop_InsertPost,
       'prop_DeletePost,
       'prop_UnionPost,
       'prop_InsertModel,
       'prop_DeleteModel,
       'prop_UnionModel,
       'prop_InsertInsert,
       'prop_InsertDelete,
       'prop_InsertUnion,
       'prop_DeleteInsert,
       'prop_DeleteDelete,
       'prop_DeleteUnion,
       'prop_UnionDeleteInsert,
       'prop_UnionUnionAssoc
     ]
 )

test_UnionUnionIdem = fsRunGen fsDefaults Naive fgen prop_UnionUnionIdem
